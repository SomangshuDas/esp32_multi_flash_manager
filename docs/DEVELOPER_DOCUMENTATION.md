...
## 16. v0.10.0 feature notes (Auto-Save, Dynamic MD5, QC, MAC, Sounds, Tags, History search)

Seven smaller, mostly-independent features landed together in v0.10.0.
Notes on the less-obvious design decisions for each:

**Auto-Save.** A single `QTimer` on `MainWindow` (`autosave_timer`),
(re)started by `_apply_autosave_interval()` at startup and whenever
Settings is saved. `_on_autosave_timeout()` is the only gate: skips if
`project_controller.current_file_path is None` (never saved before —
there's no path to write to) or if `not project_controller.dirty`
(nothing changed). Deliberately reuses `project_controller.save_project()`
— the exact same path manual Save uses — rather than a separate silent
write routine, so auto-save can never drift from what a manual save does.

**Dynamic MD5.** `FirmwareEntry.md5` is still a normal (non-persisted)
dataclass field — `to_dict()` simply omits it now. `refresh()` (called on
every load and after any firmware add) recomputes it from disk every
time and sets the transient `md5_changed` flag by comparing against
whatever `self.md5` held *before* the recompute. `from_dict()` reads a
legacy `"md5"` key (if present, from an older project file saved before
this change) purely as a one-time comparison baseline — it's never
written back out.

**QC Verification.** `HistoryEntry` gained `id`, `device_id`,
`mac_address`, `qc_status` — all with defaults, and `from_dict()` filters
incoming keys to known dataclass fields so an old or hand-edited history
JSON never raises. The QC combo box in `HistoryPanel` is only enabled for
rows whose `result == STATUS_COMPLETED`; changing it mutates the
`HistoryEntry` object directly in place (history entries are in-memory
for the session, not round-tripped through a controller).

**Device Traceability (MAC).** `extract_mac_address()` in
`read_output_parser.py` is the SAME regex `_parse_chip_info()` already
used for the Chip Info Summary tab, just factored out. `FlashController`
subscribes to each `FlashWorker`'s existing `log_line` signal
(`_capture_mac_from_log`) and keeps the first MAC match per device per
run in `self._device_macs`, consumed (and popped) in
`_on_worker_finished()` when building the `HistoryEntry`. No extra
subprocess or Read operation runs — esptool already prints the MAC during
connect on every command, `write_flash` included.

**Sounds & Notifications (`sound_player.py`).** Built on
`PySide6.QtMultimedia.QSoundEffect` (ships with the PySide6 wheel already
in `requirements.txt` — no new dependency). `play_event_sound()` checks
the global `SETTINGS_KEY_SOUNDS_ENABLED` switch, then the specific
event's own enabled flag, then plays either the user's chosen file or
`QApplication.beep()`. Every code path is wrapped so a bad/missing sound
file can never raise into the calling signal handler (flash
success/failure, batch finished, port connect/disconnect in
`main_window.py`).

**Device Groups & Tags.** `DeviceConfig.tags: list[str]`, persisted like
any other field. Filtering/sorting in `DevicePanel` is display-only — see
`MainWindow._get_sorted_devices()`, which returns a *new* sorted list
without touching `project.devices`' actual order, so nothing else
(Upload All, save order) is affected by how the table happens to be
sorted at the moment. `DeviceController.add_tag_to_devices()` is the one
place tag-adding is special-cased separately from
`apply_to_selected()`'s generic `setattr` — Batch Edit's "Tags (Add)"
field appends rather than overwrites.

**Flash History Search & Filtering.** Row-hiding via
`QTableWidget.setRowHidden()`, same pattern `DevicePanel.apply_search_filter()`
already used — `HistoryPanel._entries[i]` always corresponds to table row
`i` since rows are only ever appended or all cleared together, never
reordered or individually removed.
